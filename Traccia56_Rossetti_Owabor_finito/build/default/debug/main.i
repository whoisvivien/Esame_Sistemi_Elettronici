...
RX:
    ...
    GOTO RX ;se ((PIR1) and 07Fh), 5=1, salta il GOTO e va in RIC, altrimenti continua ad aspettare
...
TRANS: ;trasmissione
    ...
    GOTO TRANS ;se ((PIR1) and 07Fh), 4=1, salta l'istruzione successiva, altrimenti continua ad aspettare
...
INIT: ;inizializzazione dell'hardware
    ...
    BCF TRISD,0 ;azzera il bit ((TRISD) and 07Fh), 0, quindi configura il Pin PORTD come uscita
    BANKSEL PORTD ;PORTD General Purpose I/O Pin bit
    BCF PORTD,0 ;azzerra il bit ((PORTD) and 07Fh), 0, quindi spegne il LED

    ;EUSART
    ;INIZIALIZZAZIONE EUSART per trasmissione
    BANKSEL TXSTA ;EUSART Transmit Status and Control Register
    MOVLW 00100100B ;((TXSTA) and 07Fh), 5=1 (Transmit enabled), ((TXSTA) and 07Fh), 4=0 (Asynchronous mode), ((TXSTA) and 07Fh), 2=1 (High speed asynchronous mode)
    MOVWF TXSTA ;imposta il registro TXSTA

    BANKSEL RCSTA ;EUSART Receive Status and Control Register
    MOVLW 10010000B ;((RCSTA) and 07Fh), 7=1 (porta seriale abilitata), ((RCSTA) and 07Fh), 4=1 (ricezione abilitata)
    ...
    CLRF BAUDCTL ;azzera il registro, allora ((BAUDCTL) and 07Fh), 3=0 (8-bit Baud Rate Generator is used)

    BANKSEL SPBRG ;insieme al bit ((TXSTA) and 07Fh), 2 determinano il Baud Rate
...
INVIA_CARATTERE: ;routine per inviare un singolo carattere via USART
    BANKSEL PIR1 ;il registro PIR1 contiene i flag bits delle interruzioni periferiche
    BTFSS PIR1, PIR1_TXIF_POSITION ;aspetta finché il registro di trasmissione non è pronto
    GOTO INVIA_CARATTERE ;se ((PIR1) and 07Fh), 4=1, salta questa istruzione e continua la routine,
